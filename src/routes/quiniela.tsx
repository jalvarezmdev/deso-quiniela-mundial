...
const GROUP_ACCENT_BY_LETTER: Record<string, GroupAccentStyles> = {
  ...
  b: {
    badgeClassName: "bg-[var(--brand-secondary)]/20 text-[var(--brand-secondary)]",
  },
  c: {
    badgeClassName: "bg-[var(--accent)]/15 text-[var(--accent)]",
  },
...
function QuinielaPage() {
  ...
  const displayedPhase = useMemo(
    () =>
      resolveDisplayedPhase(activePhase, Boolean(currentUser?.isAdmin), phasePreview),
  ...
  const [previewByPhase, setPreviewByPhase] = useState<Record<KnockoutPhase, boolean>>({
    roundOf16: false,
    roundOf8: false,
    roundOf4: false,
    semifinals: false,
  ...
  const knockoutViews = useMemo(
    () =>
      buildKnockoutMatchViews(
        displayedPhase,
        matches,
        isKnockoutPhase(displayedPhase) ? previewByPhase[displayedPhase] : false,
      ),
  ...
  const currentUserPhasePredictions = useMemo(
    () =>
      state.predictions.filter(
        (prediction) =>
          prediction.userId === currentUser?.id && prediction.phase === displayedPhase,
      ),
  ...
  const savedProgress =
    totalMatchesCount === 0 ? 0 : Math.round((savedMatchesCount / totalMatchesCount) * 100);
        ...
        >
          <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
            <div className="text-base font-bold text-[var(--neutral)] md:text-lg">
              {home.flag} {teamDisplayWithAbv(match.homeTeamId)} <span className="mx-2 text-zinc-500">vs</span>{" "}
              {away.flag} {teamDisplayWithAbv(match.awayTeamId)}
            </div>
            <div className="flex items-center gap-2">
              {prediction ? (
                <Badge className="gap-1 bg-[var(--accent)]/15 text-[var(--accent)]">
                  <CheckCircle2Icon className="h-3.5 w-3.5" />
                  <span>{prediction.homeGoals}-{prediction.awayGoals}</span>
                </Badge>
              ) : (
    ...
    <RequireAuth>
      ...
      >
          ...
          <div className="flex flex-col gap-4">
            ...
            <section className="grid gap-4">
              {visibleRounds.map((round) => {
                ...
                return (
                  <div key={round} className="border-t border-[var(--line)] pt-6 first:border-t-0 first:pt-0">
        ...
        <div className="mt-6 mb-28 flex items-center justify-end gap-2">
          <Button onClick={() => void onConfirmPhase()} disabled={!canConfirmPhase}>
        ...
        <div className="fixed inset-x-0 bottom-0 z-40 border-t border-[var(--line)] bg-[var(--surface-0)]/95 px-4 py-3 backdrop-blur">
          <div className="mx-auto flex w-full max-w-6xl flex-col gap-3 md:flex-row md:items-center">
            <div className="w-full md:flex-1">
              ...
              {knockoutViews.missingCount > 0 ? (
                <p className="mb-1 text-[11px] text-zinc-400">
                  {knockoutViews.missingCount} cruces por definir para habilitar confirmacion.
                </p>
              ) : null}
